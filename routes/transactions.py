        ...
        try:
            ...
            with open(file_path, mode='r', encoding='utf-8') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if not val_id or not str(val_id).strip():
                    ...
                    sender = str(sender_val).strip() if sender_val else 'ACC-50001'
                    
                    rec_val = row.get('receiver_account')
                    receiver = str(rec_val).strip() if rec_val else 'ACC-50002'
                    
                    amt_val = row.get('amount')
                    try:
                        amount = float(amt_val) if amt_val else 10000.0
                    except (ValueError, TypeError):
                        amount = 10000.0

                    type_val = row.get('transaction_type')
                    t_type = str(type_val).strip() if type_val else 'TRANSFER'
                    
                    loc_val = row.get('location')
                    location = str(loc_val).strip() if loc_val else 'Mumbai, IN'
                    
                    hour_val = row.get('hour_of_day')
                    try:
                        hour_of_day = int(hour_val) if hour_val else 14
                    except (ValueError, TypeError):
                        hour_of_day = 14

                    new_ben_val = row.get('is_new_beneficiary')
                    try:
                        is_new_ben = int(new_ben_val) if new_ben_val else 0
                    except (ValueError, TypeError):
                        is_new_ben = 0

                    high_geo_val = row.get('is_high_risk_country')
                    try:
                        is_high_geo = int(high_geo_val) if high_geo_val else 0
                    except (ValueError, TypeError):
                        is_high_geo = 0

                    sender_cust_val = row.get('sender_customer_id')
                    sender_cust_id = str(sender_cust_val).strip() if sender_cust_val else 'CUST-1001'
